...
export default function Student({ user }) {
  ...
  function downloadMarksheet() {
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>Marksheet — ${user.name}</title>
      ...
      <div class="head"><img src="${logo}"><div><h1>KTN Digital Online School</h1><div class="sub">Education for Free · Since 2019 — Report card</div></div></div>
        ...
        return (
          <div key={i} className="card" style={{ padding: 14, marginBottom: 8, borderColor: isToday ? "var(--azure)" : "var(--line)", boxShadow: isToday ? "0 6px 18px -10px rgba(47,107,255,.4)" : undefined }}>
            ...
            {cancelled ? (
              ...
              <a href={link} target="_blank" rel="noopener noreferrer" className="btnP" style={{ marginTop: 10, width: "100%", justifyContent: "center", textDecoration: "none", background: "#1E9E5A" }}><Icon name="globe" size={15} color="#fff" sw={2.4} /> Join class</a>
            ) : null}
          </div>
        );
      })}
